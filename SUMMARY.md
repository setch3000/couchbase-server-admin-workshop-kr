# Table of contents

* [Workshop 소개](README.md)

## Couchbase의 기본 기능

* [1. Couchbase Capella 클러스터 생성](couchbase/1.-couchbase-capella.md)
* [2. 데이터 도구로 샘플 데이터 탐색하기](couchbase/2..md)
* [3. Query 최적화](couchbase/3.-query.md)
* [4. Couchbase Python SDK 활용](couchbase/4.-couchbase-python-sdk.md)

## Vector Search

* [1. Vector Search 실습 사전 준비](vector-search/1.-vector-search.md)
* [2. Vector Search 실습](vector-search/2.-vector-search.md)

## RAG(검색, 증강, 생성)&#x20;

* [1. RAG 실습 사전 준비](rag/1.-rag.md)
* [2. Couchbase + LangChain + OPEN AI로 RAG 구현 실습](rag/2.-couchbase-+-langchain-+-open-ai-rag.md)

## AI Agent

* [1. AI Agent 실습 사전 준비](ai-agent/page-1.md)
* [2. Couchbase + LangChain + OPEN AI로 AI Agent 구현 실습](ai-agent/2.-couchbase-+-langchain-+-open-ai-ai-agent.md)
